 ...
 * ordinary form post. A 10 MB base64 file/photo upload is a different
 * matter: the original body, the stringified copy made just to fingerprint
 * it, and (once the request itself serializes the body again to send) a
 * third copy would all be alive in memory at once -- a real constraint on a
 * phone. roughSize walks the body without allocating a serialized copy, so a
 * body past the threshold can be recognised as too large without ever being
 * stringified; dedup is then skipped for it entirely, so a double-tap on a
 * huge upload costs an extra upload rather than every upload paying to avoid
 * one. A body under the threshold is still stringified once, but only a
 * short hash of that string is kept, not the string itself.
 */
