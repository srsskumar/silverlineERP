...
describe("project types", () => {
  it("lists the 2 seeded types with the default workflow", async () => {
    const h = await adminHeaders();
    const res = await app.inject({
      method: "GET",
      url: "/api/v1/project-types",
    ...
    expect(body.data.map((t) => t.code).sort()).toEqual(["fieldwork", "general"]);
