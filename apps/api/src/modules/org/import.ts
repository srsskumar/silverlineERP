...
const rowSchema=z.object({type:z.enum(['district','mandal','village','site']),code:z.string().trim().min(1).max(50),name:z.string().trim().min(1).max(255),parent_code:z.string().trim().max(50).optional()});
class RowValidationError extends Error {}
const parentType={district:null,mandal:'district',village:'mandal',site:'village'} as const;
export async function registerOrgImport(app:FastifyInstance,opts:{pool:Pool;jwtSecret:string}){
 app.post('/api/v1/org/units/import',{preHandler:requirePermission(buildAuthenticate(opts),S1_PERMISSIONS.ORG_UNITS_MANAGE)},async req=>{
  ...
  return mutate(opts.pool,req,'org_unit.import','org_unit_import',async db=>{
   ...
   candidates.sort((a,b)=>a.parsed.success&&b.parsed.success?Object.keys(parentType).indexOf(a.parsed.data.type)-Object.keys(parentType).indexOf(b.parsed.data.type):0);
   for(const candidate of candidates){const {parsed,row}=candidate;if(!parsed.success){results.push({row,status:'REJECTED',message:parsed.error.issues.map(x=>`${x.path.join('.')}: ${x.message}`).join('; ')});continue;}
    ...
    try{
     ...
     const expected=parentType[v.type];let parent:string|null=null;
     if(expected){const found=await db.query("SELECT id FROM org_units WHERE org_id=$1 AND type=$2 AND code=$3 AND status='ACTIVE'",[u.orgId,expected,v.parent_code??'']);if(!found.rowCount)throw new RowValidationError(`An active ${expected} parent code is required`);parent=found.rows[0].id;}
