...
export async function registerAnalyticsRoutes(app:FastifyInstance,opts:{pool:Pool;jwtSecret:string}) {
 ...
 app.get('/api/v1/analytics/projects/:id',{preHandler:read},async req=>{
  ...
  const [summary,flow,cycles,workload,burndown]=await Promise.all([
   ...
   db.query("SELECT t.assignee_id,u.username,count(*)::int AS open,count(*) FILTER(WHERE planned_end_date<(now() AT TIME ZONE 'Asia/Kolkata')::date)::int AS overdue FROM tasks t LEFT JOIN users u ON u.id=t.assignee_id WHERE project_id=$1 AND t.status NOT IN ('DONE','CANCELLED') GROUP BY t.assignee_id,u.username ORDER BY open DESC LIMIT 100",[id]),
