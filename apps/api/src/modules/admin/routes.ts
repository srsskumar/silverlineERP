...
export async function registerAdminRoutes(app:FastifyInstance,opts:{pool:Pool;jwtSecret:string}) {
 ...
 app.patch('/api/v1/admin/settings',{preHandler:guard('admin.configure')},async req=>{
  const i=parse(z.object({name:z.string().trim().min(1).max(255).optional(),settings:z.object({timezone:z.string().refine(s=>{try{new Intl.DateTimeFormat('en',{timeZone:s});return true;}catch{return false;}}).optional(),locale:z.string().max(20).optional(),attendance_duplicate_minutes:z.number().int().min(1).max(60).optional(),attendance_future_tolerance_minutes:z.number().int().min(0).max(15).optional(),session_timeout_minutes:z.number().int().min(5).max(1440).optional(),retention_days:z.number().int().min(30).max(36500).optional(),gst_state_code:z.string().regex(/^\d{2}$/).refine(c=>c in GST_STATE_CODES,{message:'Not a GST state code'}).optional(),
