...
// Days print as a count and money as rupees; the grouping (and why LOP is a
// note rather than a deduction) is payslipView's, shared with the web slip.
const pdfLine=(l:PayslipLine)=>`${l.label}: ${l.kind==='days'?formatPayslipDays(l.value):`Rs ${l.value.toFixed(2)}`}`;
function payslipPdfLines(p:{earnings:Record<string,unknown>;deductions:Record<string,unknown>;total_deductions:string|number}):string[]{
 const v=payslipView(p.earnings,p.deductions,p.total_deductions);
 const out=['EARNINGS',...v.rates.map(pdfLine),'','DAYS',...v.days.map(pdfLine),'','DEDUCTIONS',...v.deductions.map(pdfLine)];
 return v.notes.length?[...out,'','FOR INFORMATION (not deducted again)',...v.notes.map(pdfLine)]:out;
}
