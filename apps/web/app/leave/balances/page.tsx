...
function AdjustDialog({
  ...
  if (!open) return null;
  return (
    <div role="dialog" aria-modal="true" aria-label="Adjust balance" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 px-4">
      <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-lg bg-white p-6 shadow-lg">
        <h2 className="text-base font-semibold text-slate-900">Adjust opening balance</h2>
        {saved ? (
  ...
  return (
    <AppShell>
      <RequirePermission code={PERMISSIONS.LEAVE_REQUEST}>
        <h1 className="text-xl font-bold text-slate-900">Leave balances</h1>
        <p className="mt-1 text-sm text-slate-500">Per-type balances for one employee and year. Adjustments need leave.admin.</p>
        <div className="mt-6">
          <BalancesPanel />
        </div>
      </RequirePermission>
