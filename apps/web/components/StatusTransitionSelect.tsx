...
export function StatusTransitionSelect({
  ...
  return (
    <div className="flex flex-col gap-2">
      <label htmlFor={`task-status-${taskId}`} className="text-sm font-medium text-slate-700">
        Move to
      </label>
      {terminal ? (
        ...
        <p className="text-sm text-slate-500">No transitions are available from {current}.</p>
      ) : (
        <div className="flex flex-col gap-2 sm:flex-row">
          <select
            id={`task-status-${taskId}`}
            className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-1 sm:max-w-xs"
            value={next}
            onChange={(e) => {
