...
export function AdvancedTaskFilters({project,value,onChange}:{project:string;value:ListTasksParams;onChange:(v:ListTasksParams)=>void}){
 ...
 <label className="text-sm">Assignee<select className={cls} value={value.assignee_id??''} onChange={e=>set('assignee_id',e.target.value)}><option value="">Anyone</option>{people.data?.rows.map(u=><option key={u.id} value={u.id}>{u.name??u.username}</option>)}</select></label>
 ...
 {/* Planned dates say what was promised; these say what happened. Both are
     needed — a slipped plan and a finished month are different questions. */}
 <label className="text-sm" title="Work that actually started on or after this date">Actually started from<input className={cls} type="date" value={value.started_from??''} onChange={e=>set('started_from',e.target.value)}/></label>
 <label className="text-sm" title="Work that actually started on or before this date">Actually started through<input className={cls} type="date" value={value.started_to??''} onChange={e=>set('started_to',e.target.value)}/></label>
 <label className="text-sm" title="Work marked done on or after this date">Actually finished from<input className={cls} type="date" value={value.finished_from??''} onChange={e=>set('finished_from',e.target.value)}/></label>
 <label className="text-sm" title="Work marked done on or before this date">Actually finished through<input className={cls} type="date" value={value.finished_to??''} onChange={e=>set('finished_to',e.target.value)}/></label>
