...
export function AdvancedTaskFilters({project,value,onChange}:{project:string;value:ListTasksParams;onChange:(v:ListTasksParams)=>void}){
 ...
 const cls='mt-1 w-full rounded border border-slate-300 bg-white p-2 text-sm text-slate-900';
 return <details className="rounded border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-medium">More filters</summary><div className="mt-3 grid gap-3 sm:grid-cols-3">
 ...
 </div><button type="button" className="mt-3 text-sm text-blue-700" onClick={()=>onChange({})}>Clear additional filters</button></details>;
}
