...
export function AssetAudit(){
 ...
 <div className="grid gap-4 md:grid-cols-2"><label className="block text-sm font-medium">Expected asset codes, one per line<textarea required rows={6} className="mt-1 block w-full rounded border p-2" value={expected} onChange={e=>setExpected(e.target.value)} placeholder={'SCANNER-01\nSCANNER-02'}/></label><label className="block text-sm font-medium">Observed assets: code, condition<textarea rows={6} className="mt-1 block w-full rounded border p-2" value={observed} onChange={e=>setObserved(e.target.value)} placeholder={'SCANNER-01, GOOD'}/><span className="text-xs text-slate-500">Omitted expected assets are recorded as missing.</span></label></div>
