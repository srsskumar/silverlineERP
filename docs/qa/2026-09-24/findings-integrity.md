...
| D-013 | P3 | api | survey, employees, alerts, billing | Actions between 00:00 and 05:30 IST | IST dates | `CURRENT_DATE` (the DB session is UTC; no session TimeZone is set) stamps yesterday on survey crew `assigned_on`/`released_on` (employees/routes.ts:2413, 2425), on the survey alert day-counts (jobs/surveyAlerts.ts) and in two survey duration queries. `measured-proposal` defaults `period_to` to the UTC date and dates task completions in UTC on purpose (see the comment at billing/routes.ts ~735). | FIXED (fix round 1): `orgTodaySql()` everywhere CURRENT_DATE was, plus `businessDay()` for the proposal default | 7ba3945 |
| D-014 | P3 | mobile | approvals, payables, receivables, payroll, projects, clients, pipeline | Look at an amount with paise | Same figure as web (`money()`, 2 dp) | Mobile `money()` helpers use `maximumFractionDigits: 0`, so an approver sees Rs 1,500 for Rs 1,499.60 | FIXED (fix round 1): one `formatMoney` in `apps/mobile/src/money.ts` | e10a47f |
| D-015 | P3 | api/web | expenses, reimbursement | Double-click "Reimburse" on a claim with a partial amount | One payment | The web client mints a new Idempotency-Key per call, so a double submit is two requests. For a partial amount, both fit under the outstanding cap. | FIXED (fix round 1): one key per submission (identical in-flight write reuses it) on web and mobile | cd9b188 |
...
## Fix round 1 (review of round 1)

Every item below was committed or captured as a failing test first (f11ed6d, a8af37e), and then fixed.

- **D-008, CRITICAL.** The claim lock and the reimbursed sum ran in one statement. While that statement waited for the lock, it kept its starting snapshot. A part reimbursement never touches the claim row, so it could commit unseen while the allocation waited.
  - RED: a real race between a reimbursement and an allocation left 3 of 10 claims paid 1,200 against 1,000.
  - Fixed in b9951d0 by taking the lock and reading the sum in separate statements.
- **D-007, IMPORTANT.** The first fix was wrong: `likeContains` emitted the literal text `${c}`, so any search containing `%` or `_` found nothing.
  - RED: searches for "50%" and "CEM_OPC_x" returned `[]`.
  - Fixed in ed1a794 by escaping with `!` and adding `ESCAPE '!'` to every search query.
  - Positive tests now cover both searches, and `%` alone matches only the row that contains `%`.
- **Leave and budget locks.** Both now use `FOR NO KEY UPDATE`, so rows that point at the employee or project can still be inserted while the lock is held (8ef9697).
- **D-010 money.** `toPrecision(12)` lost paise above about Rs 1e10. Line amounts are now exact integer-paise BigInt arithmetic in shared (`lineAmount`, `percentOf`, `addMoney`) (b2dce9e).
- **D-011, D-013, D-014 and D-015** are fixed as shown in the table above.
- **D-012** is still an owner decision.
