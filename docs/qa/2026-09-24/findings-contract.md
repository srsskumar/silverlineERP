...
Idempotency-Key replay (finding C-009) and cross-org depth (clean).
...
| C-009 | P2 | api | POST /api/v1/boards does not honour a body mismatch on a reused Idempotency-Key: it replays the first board's stored response instead of rejecting with 409, and the second board is silently never created | s5/routes.ts:421 — this route is not wrapped by mutationRoute() (unlike every sibling create route in the same file); it calls replayIfSeen()/storeIdempotentResponse() directly, and replayIfSeen only matches on key+method+path, never checking the stored request's hash against the new body the way mutate() (IDEMPOTENCY_CONFLICT) and mutationRoute() (IDEMPOTENCY_MISMATCH) both do. Reproduced live in idempotency-replay-contract.test.ts. Not fixed — task brief asked this dimension to be tested and any dangerous finding flagged, not remediated. | OPEN (flagged, not fixed) |
...
- **Result**: 236 of 256 write routes are mutate/mutationRoute-covered. 19 are explicitly exempted
  ...
  employees/bulk, tasks/:id/collaborators x2). **1 route is neither covered nor exempt:
  POST /api/v1/boards — see C-009.**
...
- **C-009 reproduced live**: POST /api/v1/boards with the same key and a genuinely different body
  returns 201 with the first board's response, not a 409 — and a row count check confirms no second
  board is ever created. Flagged, not fixed (see table above).
...
- Exhaustive (all 236) live idempotency replay — see above; the mechanism is proven generically and
  by direct reproduction of the one gap it has (C-009).
